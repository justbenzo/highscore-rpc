...
static NSString * const kLaunchAgentLabel = @"com.benzo.highscorerpc";

static NSString * LaunchAgentPlistPath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:
            @"Library/LaunchAgents/com.benzo.highscorerpc.plist"];
}
...
@property (strong) NSTask *helperTask;
@end
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    if (self.statusItem.button) {
        self.statusItem.button.title = @"HSRP";
        self.statusItem.button.toolTip = @"Highscore RPC Helper";
    }
...
- (void)startHelper {
    NSString *helperPath = [[NSBundle mainBundle] pathForResource:@"helper" ofType:@"js"];
    if (!helperPath) {
        NSLog(@"Helper.js not found in bundle");
        return;
    }

    [self launchHelperAtPath:helperPath];
}

- (void)launchHelperAtPath:(NSString *)path {
    self.helperTask = [[NSTask alloc] init];
    self.helperTask.launchPath = @"/usr/bin/env";
    self.helperTask.arguments = @[@"node", path];

    __weak typeof(self) weakSelf = self;
    self.helperTask.terminationHandler = ^(NSTask *task) {
        NSLog(@"Helper crashed, restarting...");
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [weakSelf launchHelperAtPath:path];
        });
    };
    ...
    @try {
        ...
        NSLog(@"Failed to launch helper: %@", ex);
    }
}
...
- (void)quit {
    if (self.helperTask && self.helperTask.isRunning) {
